...
This document reflects the **DBA-managed SQL schema** (see `init-db.sql` and the ER diagram).
The current Django backend uses ORM models and migrations that may create different physical table names.
**Schema alignment between the backend ORM and the DBA SQL scripts is tracked separately** and should be confirmed before production.

In particular:
- The SQL schema includes `license` on `datasets`.
- There is **no** `readme_content` column in the DBA SQL schema / ER diagram (as of the latest DBA docs).
