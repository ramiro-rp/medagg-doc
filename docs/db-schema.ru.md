...
---
## Источники и примечания по согласованию

Этот документ описывает **SQL-схему, поддерживаемую DBA** (см. `init-db.sql` и ER-диаграмму).
Текущий Django backend использует ORM и миграции, которые могут создавать другие физические имена таблиц.
**Согласование схемы между ORM и DBA SQL-скриптами ведётся отдельно** и должно быть подтверждено перед продакшеном.

Важно:
- В SQL-схеме есть поле `license` у таблицы `datasets`.
- В SQL-схеме / ER-диаграмме **нет** колонки `readme_content` (по последним материалам DBA).
